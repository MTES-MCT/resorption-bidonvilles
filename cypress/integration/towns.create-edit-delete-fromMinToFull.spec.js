    ...
    it('Je peux déclarer un site au minimum, le modifier au maximum, puis le supprimer', () => {
        cy.fixture('users').then(({ admin }) => {
            ...
            cy.wait('@getConfig').then(() => {
                cy.fixture('shantytowns').then(({ full: fullShantytown, min: minShantytown }) => {
                    /* ******************************************************************************
                    ...
                    ********************************************************************************* */
                    ...
                    cy.url().then((url) => {
                        ...
                        // assert
                        cy.get('.notification-content').should('contain', 'Le site a bien été modifié');
                        ...
                        cy.get('.notification.error')
                            .should('contain', 'Le site demandé n\'existe pas en base de données');
                    });
                });
            });
        });
