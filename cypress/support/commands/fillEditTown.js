...
Cypress.Commands.add('fillEditTown', (shantytown) => {
    ...
    if (shantytown.address) {
        if (cy.get('[data-cy-field=\'address\'] input').eq(0).value !== shantytown.address) {
    ...
    if (shantytown.field_type) {
        cy.get(`[data-cy-field="field_type"][data-cy-label="${shantytown.field_type}"]`).check();
    }

    // owner type
    if (shantytown.owner_type) {
        cy.get(`[data-cy-field="owner_type"][data-cy-label="${shantytown.owner_type}"]`).check();
    }
    ...
    if (shantytown.census_status) {
        cy.get(`[data-cy-field="census_status"][data-cy-label="${shantytown.census_status}"]`).check();
    }

    // census conducted at
    if (shantytown.census_conducted_at) {
        cy.get('[data-cy-field=\'census_conducted_at\']').chooseDate(shantytown.census_conducted_at);
    } else if (shantytown.census_status === 'Prévu' || shantytown.census_status === 'Réalisé') {
        cy.get('[data-cy-field=\'census_conducted_at\']').emptyDate();
    }

    // census conducted by
    if (shantytown.census_conducted_by) {
        cy.get('[data-cy-field=\'census_conducted_by\']').typeReplace(shantytown.census_conducted_by);
    } else if (shantytown.census_status === 'Prévu' || shantytown.census_status === 'Réalisé') {
    ...
    if (shantytown.population_total !== undefined && shantytown.population_total !== null) {
        cy.get('[data-cy-field=\'population_total\']').typeReplace(shantytown.population_total);
    } else {
    ...
    if (shantytown.population_couples !== undefined && shantytown.population_couples !== null) {
        cy.get('[data-cy-field=\'population_couples\']').typeReplace(shantytown.population_couples);
    } else {
    ...
    if (shantytown.population_minors !== undefined && shantytown.population_minors !== null) {
        cy.get('[data-cy-field=\'population_minors\']').typeReplace(shantytown.population_minors);
    } else {
    ...
    if (shantytown.electricity_type) {
        cy.get(`[data-cy-field="electricity_type"][data-cy-label="${shantytown.electricity_type}"]`).check();
    }

    // electricity comments
    if (shantytown.electricity_comments) {
        cy.get('[data-cy-field=\'electricity_comments\']').typeReplace(shantytown.electricity_comments);
    } else {
    ...
    if (shantytown.access_to_water) {
        cy.get(`[data-cy-field="access_to_water"][data-cy-label="${shantytown.access_to_water}"]`).check();
    }

    // water comments
    if (shantytown.water_comments) {
        cy.get('[data-cy-field=\'water_comments\']').typeReplace(shantytown.water_comments);
    } else {
    ...
    if (shantytown.access_to_sanitary) {
        cy.get(`[data-cy-field="access_to_sanitary"][data-cy-label="${shantytown.access_to_sanitary}"]`).check();
    }

    // sanitary comments
    if (shantytown.sanitary_comments) {
        cy.get('[data-cy-field=\'sanitary_comments\']').typeReplace(shantytown.sanitary_comments);
    } else {
    ...
    if (shantytown.trash_evacuation) {
        cy.get(`[data-cy-field="trash_evacuation"][data-cy-label="${shantytown.trash_evacuation}"]`).check();
    }

    // owner complaint
    if (shantytown.owner_complaint) {
        cy.get(`[data-cy-field="owner_complaint"][data-cy-label="${shantytown.owner_complaint}"]`).check();
    }

    // justice procedure
    if (shantytown.justice_procedure) {
        cy.get(`[data-cy-field="justice_procedure"][data-cy-label="${shantytown.justice_procedure}"]`).check();
    }

    // justice rendered
    if (shantytown.justice_rendered) {
        cy.get(`[data-cy-field="justice_rendered"][data-cy-label="${shantytown.justice_rendered}"]`).check();
    }

    // justice rendered by
    if (shantytown.justice_rendered_by) {
        cy.get('[data-cy-field=\'justice_rendered_by\']').typeReplace(shantytown.justice_rendered_by);
    ...
    if (shantytown.justice_rendered_at) {
        cy.get('[data-cy-field=\'justice_rendered_at\']').chooseDate(shantytown.justice_rendered_at);
    } else if (shantytown.justice_rendered === 'Oui') {
    ...
    if (shantytown.justice_challenged) {
        cy.get(`[data-cy-field="justice_challenged"][data-cy-label="${shantytown.justice_challenged}"]`).check();
    }

    // police status
    if (shantytown.police_status) {
        cy.get(`[data-cy-field="police_status"][data-cy-label="${shantytown.police_status}"]`).check();
    }

    // police requested at
    if (shantytown.police_requested_at) {
        cy.get('[data-cy-field=\'police_requested_at\']').chooseDate(shantytown.police_requested_at);
    } else if (shantytown.police_status === 'Demandé' || shantytown.police_status === 'Obtenu') {
        cy.get('[data-cy-field=\'police_requested_at\']').emptyDate();
    }

    // police granted at
    if (shantytown.police_granted_at) {
        cy.get('[data-cy-field=\'police_granted_at\']').chooseDate(shantytown.police_granted_at);
