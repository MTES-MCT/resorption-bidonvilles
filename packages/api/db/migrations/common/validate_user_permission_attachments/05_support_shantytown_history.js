...
CREATE OR REPLACE FUNCTION validate_user_permission_attachments() RETURNS trigger AS $validate_user_permission_attachments$
    ...
    BEGIN
        ...
        CASE
            WHEN entity IN ('shantytown', 'shantytown_comment', 'shantytown_justice', 'shantytown_owner', 'shantytown_actor', 'shantytown_history')
            THEN
                IF NEW.fk_plan IS NOT NULL OR NEW.fk_action IS NOT NULL THEN
                    RAISE EXCEPTION 'on ne peut pas rattacher une action à une permission de type %', entity;
                END IF;

            WHEN entity IN ('action', 'action_comment', 'action_finances', 'plan', 'plan_finances', 'plan_comment')
            THEN
            ...
            THEN
                IF NEW.fk_shantytown IS NOT NULL OR NEW.fk_plan IS NOT NULL OR NEW.fk_action IS NOT NULL THEN
                    RAISE EXCEPTION 'on ne peut pas rattacher un site ou une action à une permission de type %', entity;
                END IF;
