import shantytownModelFindOne from '#server/models/shantytownModel/findOne';

export async function checkReadPermission(req, res, next) {
    ...
    try {
        req.shantytown = await shantytownModelFindOne(req.user, parseInt(req.params.id, 10));
    } catch (error) {
        res.status(500).send({
