...
export default () => sequelize.query(
    ...
    FROM (
        ...
        UNION
        (
            SELECT user_mobile_navigation_logs.fk_user, DATE_TRUNC('week', user_mobile_navigation_logs.datetime) AS monday
        FROM user_mobile_navigation_logs
        LEFT JOIN users ON user_mobile_navigation_logs.fk_user = users.user_id
        WHERE user_mobile_navigation_logs.datetime < DATE_TRUNC('week', NOW()) 
        )
    ) t
