...
export default (attachments: Attachment[], userPermissionId, transaction) => Promise.all(
    attachments.map(attachment => sequelize.query(
        `INSERT INTO user_permission_attachments(fk_user_permission, fk_region, fk_departement, fk_epci, fk_city, fk_shantytown, fk_action)
            VALUES (:permission_id, :region, :departement, :epci, :city, :shantytown, :action)`,
        {
            transaction,
            replacements: {
