 ...
 */
module.exports = async (userLocation, permissions, location, locationType, locationCode, entities = ['shantytownCreation', 'shantytownClosing', 'shantytownUpdate', 'shantytownComment', 'highCovidComment', 'user'], numberOfActivities = 10, lastDate = null) => {
    const promises = [];
    ...
    if (shantytownFilter.length > 0) {
        promises.push(shantytownModel.getHistory(userLocation, permissions, location, locationType, locationCode, shantytownFilter, numberOfActivities, lastDate));
    }
    if (entities.includes('shantytownComment')) {
        promises.push(shantytownCommentModel.getHistory(userLocation, permissions, location, locationType, locationCode, numberOfActivities, lastDate, entities.includes('onlyCovid')));
    }
    if (entities.includes('highCovidComment')) {
        promises.push(highCovidCommentModel.getHistory(userLocation, permissions, location, locationType, locationCode, numberOfActivities, lastDate));
    }
    if (entities.includes('user')) {
        promises.push(userModel.getHistory(locationType, locationCode, numberOfActivities, lastDate));
    }
