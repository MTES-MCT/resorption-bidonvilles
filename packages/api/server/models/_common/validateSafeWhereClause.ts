...
export default function validateSafeWhereClause(whereClause: string): void {
    ...
    // Ajout de ' et | dans les caractères autorisés pour permettre les chaînes littérales (ex: '2|%')
    ...
    // Ajout de mots clés nécessaires aux subqueries et array operations
    const allowedWords = new Set(['AND', 'OR', 'IN', 'NOT', 'IS', 'NULL', 'TRUE', 'FALSE', 'ILIKE', 'LIKE', 'BETWEEN', 'ASC', 'DESC', 'LOWER', 'UPPER', 'ASIN', 'SQRT', 'POWER', 'SIN', 'COS', 'EXISTS', 'SELECT', 'FROM', 'WHERE', 'UNNEST', 'AS']);
