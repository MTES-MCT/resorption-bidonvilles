...
const signature = require('#server/mails/old/signature');
const { frontUrl } = require('#server/config');

module.exports = (administrator, activationLink, expiracyDate) => ({
    Subject: '[Résorption-bidonvilles] - Activer mon compte',

    TextPart: `Bonjour,

    Bienvenue sur Résorption-bidonvilles, plateforme d'information, de partage et de pilotage des acteurs de la résorption des bidonvilles.

    Votre accès a été ouvert par l'administrateur local ${administrator.first_name} ${administrator.last_name.toUpperCase()} (${administrator.organization.name}).

    Activez cet accès dès maintenant pour utiliser la plateforme. Attention, le lien est valable 7 jours, jusqu'au ${dateToString(expiracyDate, true)}}.
                        ...
                        <tbody>
                            <tr>
                                <td bgcolor="#ffffff">
                                    Bonjour,<br/>
                                    <br/>
                                    Bienvenue sur <em><a style="color: #000F8C; text-decoration: none;" href="${frontUrl}">Résorption-Bidonvilles</a></em>, plateforme d'information, de partage et de pilotage des acteurs de la résorption des bidonvilles.<br/>
                                    <br/>
                                    Votre accès a été ouvert par l'administrateur local ${administrator.first_name} ${administrator.last_name.toUpperCase()} (${administrator.organization.name}).<br/>
                                    <br/>
                                    Activez cet accès dès maintenant pour utiliser la plateforme. Attention, le lien est valable 7 jours, jusqu'au ${dateToString(expiracyDate, true)}.<br/>
                                    <br/>
