        ...
        async export(req, res, next) {
            ...
            if (!can(req.user).do('export', 'shantytown_history').on(location) && !exportIsToday) {
                return res.status(400).send({
                    ...
                    response: {
                        error: {
                            user_message: 'Vous n\'êtes pas autorisé(e) à exporter les données d\'une date passée',
                            developer_message: 'the user is not allowed to export data from the past',
                        },
                    },
                });
            }
            ...
            const buffer = await createExport(
                ...
                sections,
                shantytowns,
                moment(req.query.date).format('DD/MM/YYYY'),
            );
