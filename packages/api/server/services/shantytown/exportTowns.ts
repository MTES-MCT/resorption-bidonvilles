...
export default async (user: User, location: Location, closedTowns: boolean = false, date: Date = new Date()) => {
    ...
    const buffer = await generateExportFile(user, data, locations, closedTowns, date);
