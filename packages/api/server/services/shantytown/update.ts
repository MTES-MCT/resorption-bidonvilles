...
function buildUpdatePayload(shantytown, originalShantytown) {
    return {
        ...
        population_updated_at: checkPopulationUpdate(originalShantytown, shantytown),
        ...
        ...(shantytown.living_conditions_version === 2 ? buildLivingConditionsV2Payload(shantytown) : {}),
    };
}
...
async function updateShantytownData(user, shantytownId: number, payload, transaction) {
    try {
...
export default async function update(shantytown, user, decreeAttachments: DecreeAttachments): Promise<Shantytown> {
    ...
    await sendReinstallationNotification(originalShantytown, updatedShantytown, user);
