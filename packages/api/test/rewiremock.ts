...
rewiremock.passBy(name => name.slice(-2) === '.d' || /\/ServiceError\.?|\/moment\.?|server\/config\.ts/.test(name) || /fromMimeToExtension/.test(name) || /server\/utils\/comment\//.test(name) || /sendMailsWithConcurrencyLimit/.test(name));
