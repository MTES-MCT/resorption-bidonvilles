import { City } from '#server/models/geoModel/Location.d';
import { Shantytown } from '#root/types/resources/Shantytown.d';
import locations from './location';
import fakeUser from './user';

const defaultLocation = locations.paris.city();

export function creationInput() {
    return {
        name: globalThis.generate('string'),
        latitude: globalThis.generate('string'),
        longitude: globalThis.generate('string'),
        address: globalThis.generate('string'),
        detailed_address: globalThis.generate('string'),
        built_at: globalThis.generate('string'),
        population_total: globalThis.generate('string'),
        population_total_females: globalThis.generate('string'),
        population_couples: globalThis.generate('string'),
        population_minors: globalThis.generate('string'),
        population_minors_girls: globalThis.generate('string'),
        population_minors_0_3: globalThis.generate('string'),
        population_minors_3_6: globalThis.generate('string'),
        population_minors_6_12: globalThis.generate('string'),
        population_minors_12_16: globalThis.generate('string'),
        population_minors_16_18: globalThis.generate('string'),
        minors_in_school: globalThis.generate('string'),
        caravans: globalThis.generate('string'),
        huts: globalThis.generate('string'),
        tents: globalThis.generate('string'),
        cars: globalThis.generate('string'),
        mattresses: globalThis.generate('string'),
        field_type: globalThis.generate('string'),
        owner_type: globalThis.generate('string'),
        is_reinstallation: globalThis.generate('string'),
        reinstallation_comments: globalThis.generate('string'),
        reinstallation_incoming_towns_full: [{ id: 1 }, { id: 2 }, { id: 3 }],
        citycode: globalThis.generate('string'),
        declared_at: globalThis.generate('string'),
        census_status: globalThis.generate('string'),
        census_conducted_at: globalThis.generate('string'),
        census_conducted_by: globalThis.generate('string'),
        social_origins: [2],

        owner_complaint: globalThis.generate('string'),
        owner: [{
            ownerId: 1,
            name: 'Jean Bon',
            type: 1,
        }],
        justice_procedure: globalThis.generate('string'),
        justice_rendered: globalThis.generate('string'),
        justice_rendered_by: globalThis.generate('string'),
        justice_rendered_at: globalThis.generate('string'),
        justice_challenged: globalThis.generate('string'),
        police_status: globalThis.generate('string'),
        police_requested_at: globalThis.generate('string'),
        police_granted_at: globalThis.generate('string'),
        bailiff: globalThis.generate('string'),

        living_conditions_version: 2,
        water_access_type: 'autre',
        water_access_type_details: 'comment',
        water_access_is_public: false,
        water_access_is_continuous: false,
        water_access_is_continuous_details: 'comment',
        water_access_is_local: true,
        water_access_is_close: true,
        water_access_is_unequal: true,
        water_access_is_unequal_details: 'comment',
        water_access_has_stagnant_water: true,
        water_access_comments: 'comment',

        sanitary_open_air_defecation: true,
        sanitary_toilet_types: ['latrines', 'toilettes_chimiques'],
        sanitary_working_toilets: true,
        sanitary_toilets_are_inside: true,
        sanitary_toilets_are_lighted: true,
        sanitary_hand_washing: true,

        electricity_access: true,
        electricity_access_is_unequal: true,
        electricity_access_types: ['electrogene', 'reseau_urbain', 'installation_du_bati'],

        trash_is_piling: true,
        trash_evacuation_is_close: true,
        trash_evacuation_is_safe: true,
        trash_evacuation_is_regular: true,
        trash_bulky_is_piling: true,

        pest_animals_presence: true,
        pest_animals_details: 'comment',

        fire_prevention_diagnostic: true,
    };
}

export const updateInput = () => ({
    address: globalThis.generate('string'),
    name: globalThis.generate('string'),
    built_at: globalThis.generate('string'),
    declared_at: globalThis.generate('string'),
    field_type: 3,
    detailed_address: globalThis.generate('string'),
    owner_type: 3,
    owner: globalThis.generate('string'),
    population_total: 180,
    population_total_females: 90,
    population_couples: 75,
    population_minors: 27,
    population_minors_girls: 12,
    population_minors_0_3: 3,
    population_minors_3_6: 4,
    population_minors_6_12: 10,
    population_minors_12_16: 5,
    population_minors_16_18: 5,
    social_origins: [2],
    caravans: 95,
    huts: 20,
    tents: 10,
    cars: 5,
    mattresses: 3,
    census_status: 'none',
    census_conducted_by: null,
    is_reinstallation: true,
    reinstallation_comments: globalThis.generate('string'),
    owner_complaint: null,
    justice_procedure: null,
    justice_rendered: null,
    justice_challenged: null,
    evacuation_under_time_limit: null,
    insalubrity_order: false,
    insalubrity_order_displayed: null,
    police_status: null,
    existing_litigation: null,
    update_to_date: 1,
    updated_at: globalThis.generate('string'),
    water_access_type: globalThis.generate('string'),
    water_access_type_details: null,
    water_access_is_public: false,
    water_access_is_continuous: true,
    water_access_is_continuous_details: null,
    water_access_is_local: true,
    water_access_is_close: true,
    water_access_is_unequal: false,
    water_access_is_unequal_details: null,
    water_access_has_stagnant_water: false,
    water_access_comments: globalThis.generate('string'),
    sanitary_open_air_defecation: null,
    sanitary_toilet_types: [],
    sanitary_access_working_toilets: null,
    sanitary_access_toilets_are_inside: null,
    sanitary_access_toilets_are_lighted: null,
    sanitary_access_hand_washing: null,
    electricity_access: true,
    electricity_access_types: ['installation_du_bati'],
    electricity_access_is_unequal: null,
    trash_is_piling: true,
    trash_evacuation_is_close: true,
    trash_evacuation_is_safe: true,
    trash_evacuation_is_regular: true,
    trash_bulky_is_piling: false,
    pest_animals_presence: null,
    pest_animals_details: null,
    fire_prevention_diagnostic: null,
    living_conditions_version: 2,
    citycode: '44162',
    coordinates: '47.229040269093204,-1.638936996459961',
    existingAttachments: [],
    newAttachments: [],
    preparatory_phases_toward_resorption: [],
    terminated_preparatory_phases_toward_resorption: {
        sociological_diagnosis: globalThis.generate('string'),
        social_assessment: globalThis.generate('string'),
        political_validation: globalThis.generate('string'),
        contract_preparation: globalThis.generate('string'),
    },
    updated_without_any_change: false,
    attachments: [],
    city: {
        type: 'city',
        region: { name: 'Pays de la Loire', code: '52' },
        departement: { name: 'Loire-Atlantique', code: '44' },
        epci: { name: 'Nantes Métropole', code: '244400404' },
        city: { name: 'Saint-Herblain', code: '44162', main: null },
    },
    latitude: 47.229040269093204,
    longitude: -1.638936996459961,
    field_type_full: {
        id: 3, label: 'Terrain', color: '#9D6018', position: 0,
    },
    owner_type_full: { id: 3, label: 'Privé', position: 1 },
    reinstallation_incoming_towns_full: [],
    census_conducted_at: null,
    minors_in_school: null,
    social_origins_full: [{ id: 2, uid: 'european', label: 'Ressortissants européens' }],
    justice_rendered_by: null,
    justice_rendered_at: null,
    administrative_order_decision_rendered_by: null,
    insalubrity_order_type: null,
    insalubrity_order_by: null,
    insalubrity_parcels: null,
    police_requested_at: null,
    police_granted_at: null,
    bailiff: null,
    active_preparatory_phases_toward_resorption: [],
    id: '1',
});

export function serialized(location: City = defaultLocation, override = {}): Shantytown {
    const {
        city, epci, departement, region,
    } = location;

    const shantytown: Shantytown & { hasInitialResorptionPhases?: boolean } = {
        type: 'shantytown',
        id: 1,
        updatedWithoutAnyChange: false,
        city: {
            ...city,
            latitude: 0,
            longitude: 1,
        },
        epci,
        departement: {
            ...departement,
            latitude: 0,
            longitude: 1,
            chieftown: {
                code: '10',
                name: '10',
                latitude: 0,
                longitude: 1,
            },
        },
        region: {
            ...region,
            latitude: 0,
            longitude: 1,
            chieftown: {
                code: '10',
                name: '10',
                latitude: 0,
                longitude: 1,
            },
        },
        name: null,
        status: 'open',
        closingContext: null,
        latitude: 1,
        longitude: 0,
        declaredAt: null,
        builtAt: Date.now(),
        isReinstallation: false,
        reinstallationComments: null,
        reinstallationIncomingTowns: [],
        closedAt: null,
        address: 'adresse',
        addressDetails: null,
        addressSimple: 'Adresse',
        attachments: [],
        usename: 'Nom',
        populationTotal: null,
        populationTotalFemales: null,
        populationMinorsGirls: null,
        populationCouples: null,
        populationMinors: null,
        populationMinors0To3: null,
        populationMinors3To6: null,
        populationMinors6To12: null,
        populationMinors12To16: null,
        populationMinors16To18: null,
        minorsInSchool: null,
        populationUpdatedAt: Math.floor(Date.now() / 1000),
        caravans: null,
        huts: null,
        tents: null,
        cars: null,
        mattresses: null,
        livingConditions: {
            version: 2,
            electricity: {
                status: {
                    status: 'unknown',
                    positive: [],
                    negative: [],
                    unknown: ['access', 'acces_types', 'access_is_unequal'],
                },
                access: null,
                access_types: [],
                access_is_unequal: null,
            },
            water: {
                status: {
                    status: 'unknown',
                    positive: [],
                    negative: [],
                    unknown: [
                        'access_type',
                        'access_type_details',
                        'access_is_public',
                        'access_is_continuous',
                        'access_is_continuous_details',
                        'access_is_local',
                        'access_is_close',
                        'access_is_unequal',
                        'access_is_unequal_details',
                        'access_has_stagnant_water',
                        'access_comments',
                    ],
                },
                access_type: null,
                access_type_details: null,
                access_is_public: null,
                access_is_continuous: null,
                access_is_continuous_details: null,
                access_is_local: null,
                access_is_close: null,
                access_is_unequal: null,
                access_is_unequal_details: null,
                access_has_stagnant_water: null,
                access_comments: null,
            },
            trash: {
                status: {
                    status: 'unknown',
                    positive: [],
                    negative: [],
                    unknown: [
                        'is_piling',
                        'evacuation_is_close',
                        'evacuation_is_safe',
                        'evacuation_is_regular',
                        'bulky_is_piling',
                    ],
                },
                is_piling: null,
                evacuation_is_close: null,
                evacuation_is_safe: null,
                evacuation_is_regular: null,
                bulky_is_piling: null,
            },
            sanitary: {
                status: {
                    status: 'unknown',
                    positive: [],
                    negative: [],
                    unknown: [
                        'open_air_defecation',
                        'working_toilets',
                        'toilet_types',
                        'toilets_are_inside',
                        'toilets_are_lighted',
                        'hand_washing',
                    ],
                },
                open_air_defecation: null,
                working_toilets: null,
                toilet_types: [],
                toilets_are_inside: null,
                toilets_are_lighted: null,
                hand_washing: null,
            },
            pest_animals: {
                status: {
                    status: 'unknown',
                    positive: [],
                    negative: [],
                    unknown: ['presence', 'details'],
                },
                presence: null,
                details: null,
            },
            fire_prevention: {
                status: {
                    status: 'unknown',
                    positive: [],
                    negative: [],
                    unknown: ['diagnostic'],
                },
                diagnostic: null,
            },
        },
        censusStatus: 'none',
        censusConductedBy: null,
        censusConductedAt: null,
        fieldType: {
            id: 1,
            label: 'Inconnu',
        },
        socialOrigins: [],
        comments: [],
        actors: [],
        actions: [],
        closingSolutions: [],
        closedWithSolutions: null,
        changelog: [],
        createdAt: Date.now(),
        updatedAt: null,
        createdBy: fakeUser(),
        updatedBy: fakeUser(),
        heatwaveStatus: false,
        resorptionTarget: null,
        completionRate: 0.5,
        distance: null,
        preparatoryPhasesTowardResorption: [],
        hasInitialResorptionPhases: false,
    };

    return {
        ...shantytown,
        ...override,
    };
}

export default serialized;
