const LABELS = {
    water: {
        continuousAccess(status) {
            if (status === "negative") {
                return {
                    text: "Accès non continu, c’est-à-dire qu’il varie en qualité et quantité dans la journée et les saisons, sans limite dans le temps",
                };
            }

            return {
                text: "Accès continu, c’est-à-dire qu’il ne varie pas en qualité et quantité dans la journée et les saisons, sans limite dans le temps",
            };
        },
        distance(status) {
            if (status === "unknown") {
        ...
        roadsToCross(status) {
            ...
            return {
                text: "Pas de franchissement de rue ou route pour accéder aux points d’eau",
            };
        },
    ...
    trash: {
        cansOnSite(status, town) {
            ...
            return {
                text: "Nombre de poubelles / bennes à proximité immédiate du site (moins de 100 mètres)",
            };
        },
        evacuationRegular(status) {
            ...
            return {
                text: "Collecte régulière des poubelles / bennes. C’est-à-dire au moins une fois par semaine, à partir d’un point de dépôt spécialement aménagé sur le site ou à proximité immédiate",
            };
        },
        accumulation(status) {
            if (status === "negative") {
    ...
    firePrevention: {
        diagnostic(status) {
            if (status === "negative") {
                return {
                    text: "Pas de diagnostic prévention incendie réalisé par le SDIS",
                };
            }
        ...
        devices(status) {
            if (status === "negative") {
                return {
                    text: "Pas de mesure spécifiques (formation, extincteurs...) en place",
                };
            }
...
LABELS.water.accessIsClose = (status) => {
    ...
    if (status === "negative") {
        return {
            text: "La plus grande distance point d'eau / habitation est de plus de 200 mètres",
        };
    }

    return {
        text: "La plus grande distance point d'eau / habitation est de moins de 200 mètres",
    };
};
...
LABELS.trash.evacuationIsSafe = (status) => {
    if (status === "negative") {
        return {
            text: "Les dispositifs de ramassage des ordures ménagères ne sont pas en bon état / sécurisés",
        };
    }
    ...
    return {
        text: "Les dispositifs de ramassage des ordures ménagères sont en bon état et sécurisés",
    };
};
LABELS.trash.bulkyIsPiling = (status) => {
    if (status === "positive") {
        return {
            text: "Pas d'accumulation de déchets type encombrants (hors ferraille) / gravats",
        };
    }

    return {
        text: "Accumulation de déchets type encombrants (hors ferraille) / gravats",
    };
};
