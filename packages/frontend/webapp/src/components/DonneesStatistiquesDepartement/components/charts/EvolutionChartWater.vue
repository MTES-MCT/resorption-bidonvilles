<template>
    <section>
        <h1 class="font-bold text-primary text-lg">Accès à l'eau</h1>
        <p class="mt-2">
        ...
        <div class="flex mt-4 space-x-6">
            ...
            <ChartBigFigure
                icon="faucet-drip"
                :figure="data.figures.access_to_water.value"
                :evolution="data.figures.access_to_water.evolution * 100"
                invert
                >Nombre de personnes avec accès à l'eau</ChartBigFigure
            >
        </div>
...
const data = departementMetricsStore.evolution.data.livingConditions.water;
...
const chartData = computed(() => ({
    ...
    datasets: [
        ...
        {
            label: "Nombre de personnes avec accès à l'eau",
            backgroundColor: ["#FFB7A5"],
            fill: true,
            data: data.charts.access_to_water,
        },
    ],
}));
const chartOptions = {
