    ...
    <PrivateContainer class="py-16">
      <div v-if="state === 'loaded' && user">
        ...
        <div class="flex">
          <div class="w-1/3">
            ...
            <router-link
              ...
              >Modifier les informations de l'utilisateur</router-link
            >
            <div class="border-1 border-G100 w-16 my-4"></div>
            <UserValidateAccessStatus :user="user" :loggedUser="loggedUser" />
          </div>
          ...
          <div class="ml-24 w-2/3">
            ...
            >
              <UserValidateComment :user="user"></UserValidateComment>
            </div>
            <div class="bg-yellow-200 p-4 mb-12" v-if="user.status === 'new'">
              Ne jamais envoyer un accès si vous avez un doute sur l’identité de
              la personne.
            </div>
          </div>
        </div>
        <div class="flex justify-end">
          ...
          >
            <Button
              v-if="user.status === 'new' && user.user_accesses.length === 0"
              class="mr-4"
              variant="primary"
          ...
          <div class="ml-4" v-if="loggedUser.role_id === 'national_admin'">
            <Button
        ...
        <div class="italic mt-4 flex justify-end h-8">
          ...
          <div v-if="isHoverSendAccess">
            L’utilisateur va recevoir un mail avec un lien d'activation.
          </div>
          <div v-if="isHoverDisableAccess">
            L’utilisateur ne pourra plus se connecter, les données partagées
            seront conservées.
          </div>
        </div>

        <div v-if="validation.error" class="text-error flex justify-end">
          {{ validation.error }}
        </div>
      </div>
...
export default {
  ...
  computed: {
    ...
    permissions() {
      return this.$store.state.config.configuration.permissions_description;
    },
    ...
    async validate() {
      ...
      try {
        await sendActivationLink(this.$route.params.id, {
          options: {
            close_shantytown: this.checkedOptions.includes("close_shantytown"),
            create_and_close_shantytown: this.checkedOptions.includes(
              "create_and_close_shantytown"
            ),
            access_justice: this.checkedOptions.includes("access_justice")
          }
        });
    ...
    async copyActivationLink() {
      ...
      try {
        ...
        notify({
          ...
          type: "success",
          title: "Succès",
          text: "Le lien d'activation a été copié dans le presse-papier"
        });
      } catch (error) {
        alert(
