<template>
    <ul class="list-none ml-4">
        <TableauDeBordSiteFiltre v-for="item in items" :key="item.id" :item="item" @click="setFilter(item.id)">{{
                item.label
        }} ({{ dashboardStore.towns.data[item.id].length }})
            <template v-if="item.sublabel"><br /><span class="inline-block w-full font-normal text-center"
                    :class="item.active ? 'visible' : 'invisible'">{{ item.sublabel }}</span></template>
