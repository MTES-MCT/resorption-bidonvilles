<template>
    <div>
        ...
        <div v-else-if="townsStore.error">
            <Warning :autohide="false">Une erreur est survenue au chargement de la liste des
                sites</Warning>
        </div>
        <div v-else>
            <TabList :tabs="tabs" v-model="currentTab" class="mb-3" />
            <TextInput name="incoming_towns_search" prefixIcon="search"
            ...
            <Tableau :columns="filteredColumns" :data="currentTabData">
                <template v-slot:cell="{ content, column }">
                    <Checkbox v-if="column === 'checkbox'" variant="checkbox" v-model="checked[content]"
                        @change="onCheckChange(content)" />
                    <TypeDeSite :fieldType="content" v-else-if="column === 'fieldType'" />
                    <span v-else-if="column === 'population'" class="block text-right">
                        <NombreHabitants v-if="content !== null" class="justify-end" :population="content" />
                        <span v-else class="text-G600 italic">N/C</span>
                    </span>
                    <span v-else-if="column === 'closedAt'" class="inline-block italic text-center w-full">
                        {{ content }}
                    </span>
...
const checked = ref((value.value || []).reduce((acc, id) => {
    acc[id] = true;
    return acc;
}, {}));
