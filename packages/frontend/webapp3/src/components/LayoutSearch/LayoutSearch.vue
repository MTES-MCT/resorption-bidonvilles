<template>
    <Layout v-bind="$attrs">
        <template v-slot:banner>
            <div class="pt-6 text-center print:hidden" :class="showReset ? 'pb-4' : 'pb-10'">
                <h1 class="text-lg xl:text-xl font-bold">{{ searchTitle }}</h1>
                <ContentWrapper class="mt-3" size="medium">
                    <div class="flex items-center space-x-2">
                        <InputLocation class="flex-1" name="territorial_collectivity" :placeholder="searchPlaceholder"
                            withoutMargin :allowFreeSearch="allowFreeSearch" v-model="inputLocation" />
