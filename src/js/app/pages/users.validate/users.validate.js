    ...
    methods: {
        ...
        load() {
            ...
            get(this.$route.params.id)
                .then((user) => {
                    this.user = user;
                    this.checkedOptions = this.options.reduce((acc, option) => Object.assign(acc, {
                        [option.id]: user.permission_options.indexOf(option.id) !== -1,
                    }), {});
        ...
        hasPermissionsFor(level) {
            return this.permission && this.permission[`${level}_permissions`] && this.permission[`${level}_permissions`].length > 0;
        },
